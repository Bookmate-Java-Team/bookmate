...
public class ReviewService {
  ...
  public Review createReview(Long userId, ReviewRequestDto reviewRequestDto) {
    ...
    Book book = findBookById(reviewRequestDto.getBook().getId());

    if (userBookRecordRepository.existsByBookAndUser(reviewRequestDto.getBook(), user)) {
      throw new ReviewDuplicateException(reviewRequestDto.getBook().getIsbn());
    }

    UserBookRecord userBookRecord = userBookRecordRepository.save(
        UserBookRecord.builder().user(user).book(book).build());
  ...
  @Transactional
  public Review updateReview(Long userId, Long id, ReviewRequestDto reviewRequestDto) {
    User user = findUserById(userId);
  ...
  private Review findReviewByUserBookRecord(UserBookRecord userBookRecord) {
    return reviewRepository.findByUserBookRecord(userBookRecord).orElseThrow(() -> new ReviewNotFoundException(userBookRecord.getId()));
  }
